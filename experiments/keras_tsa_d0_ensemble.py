...
def build_sktime_champion_model(n_inputs):
...
def build_sktime_challenger_model(n_inputs):
  model = keras.Sequential()
  kernel_initializer_0 = keras.initializers.RandomUniform(minval=0.01, maxval=0.02, seed=None)
  bias_initializer_0 = keras.initializers.RandomUniform(minval=0.01, maxval=0.21, seed=None)
  model.add(keras.layers.Dense(32, activation="linear",input_shape=(1, n_inputs),
            kernel_initializer=kernel_initializer_0,
            bias_initializer=bias_initializer_0))
...
def ktrain(d:dict, epochs=200):
    ...
    model = build_sktime_challenger_model(n_inputs=d['n_input'])
...
def compare_to_previous():
   d = cached_surrogate_data(fname='tsa_balanced_d0_ensemble', k=1, n_real=50, n_samples=150, n_warm = 100, n_input=80)
