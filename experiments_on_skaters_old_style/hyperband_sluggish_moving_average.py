...
def tunertrain(d, n_input, epochs):
    ...
    def build_model(hp, n_inputs):
        ...
        model.add(keras.layers.Dense(
                        ...
                        activation=hp.Choice('activation_1',['softsign']) ))
        # Third layer
        model.add( keras.layers.Dense(
                        units=hp.Choice('units_2', [2,4,8,16,32]),
                        activation=hp.Choice('activation_2',['linear','relu','tanh','sigmoid','selu','elu','exponential'])))
        # Final layer
...
if __name__=='__main__':
   n_inputs = 80
   epochs = 500
   d = cached_surrogate_data(fname='sluggish_moving_average', k=1, n_real=50, n_samples=150, n_warm=100, n_input=n_inputs)
