...
def keras_cnn_shallow_7(us, n_inputs: int):
    layer_choices = {1: 30}
    return keras_cnn_factory_light(us=us, n_inputs=n_inputs, layer_choices=layer_choices, filters_low=4, filters_high=64)
...
def keras_cnn_factory_light(us, n_inputs: int, layer_choices: dict, filters_low: int, filters_high: int):
    """ Maps cube onto model and search params """
    search_params = {'epochs': 10000,
                     'patience': 6,
                     'jiggle_fraction': 0.6}
    n_search_params = 0
    n_cnn_layers = choice_from_dict(us[n_search_params], layer_choices)
...
KERAS_CNN_MODELS = [keras_cnn_shallow_7, keras_cnn_shallow_10, keras_cnn_deep_14]
